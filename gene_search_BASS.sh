...
if [[ $# -eq 0 ]]; then
	echo -e "## Gene Search ## - Arguments please!  Try ./geneSearch /this/input/folder GENESYMBOL \nUse on BASS  annotated score files"
	exit
fi

if [[ $# -lt 2 ]]; then
        echo -e "## Gene Search ## - More Arguments please!  Try ./geneSearch /this/input/folder GENESYMBOL \nUse on BASS  annotated score files"
        exit
fi

if [[ ! -d ${loc} ]]; then
	echo -e "## Gene Search ## - Input folder provided does not exist - Exiting Now"
	exit
fi
...
if [ -d ".genesearch_temp" ]; then
	echo -e "## Gene Search ## - Temp folder exists - folder not generated\n"
else
...
echo -e "## Gene Search ## - Searching for ${gene}..."
...
echo "## Gene Search ## - $(cat input.list | wc -l) total files searched..."
mv geneSearch_${gene}_results.txt ../
cd ../

if [ -d ".genesearch_temp" ]; then
        echo -e "## Gene Search ## - Results folder exists - folder not generated"
else
        mkdir gene_search_results/
fi

mv geneSearch_${gene}_results.txt gene_search_results/
rm -r .genesearch_temp

echo -e "## Gene Search ## - Search Complete"
